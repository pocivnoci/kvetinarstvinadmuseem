...
--  DPH v nákupu
--
--  Krám není plátce DPH a velkoobchod fakturuje bez DPH, takže kalkulačka
--  k nákupním cenám připočítává DPH sama. Sazbu si floristka nastaví
--  v Nastavení (výchozí 21 %).
--
--  Admin to nastavení poslal už od commitu d272b25, jenže tabulka settings
--  pro něj neměla sloupec a obě funkce vyjmenovávají sloupce ručně. Změna
--  se proto zahodila při každém uložení a po načtení z databáze byla
--  zpátky výchozí hodnota.
--
--  Obě funkce jsou znovu v celku, aby se daly přečíst na jednom místě.
--  Proti 0005 přibyl jen klíč purchaseVatRate v bloku settings.
-- ════════════════════════════════════════════════════════════════════

alter table public.settings
  add column purchase_vat_rate numeric(5, 2) not null default 21
    check (purchase_vat_rate >= 0 and purchase_vat_rate < 100);
