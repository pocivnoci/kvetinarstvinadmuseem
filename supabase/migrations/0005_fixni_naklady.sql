...
--
--  Nájem, energie, účetní. Zadají se jednou a počítají se do každého
--  měsíce samy. Bez nich admin tvrdil, že zisk se rovná tržbám: v
--  databázi je 266 tisíc příjmů a nula přijatých faktur, protože nájem
--  na Vinohradské se neměl kam zapsat.
--
--  Měsíc se drží jako datum prvního dne (valid_from, valid_to), ne jako
--  text „YYYY-MM": datum databáze umí porovnávat a hlídat.
--
--  Obě funkce jsou znovu v celku, aby se daly přečíst na jednom místě.
--  Proti 0004 přibyl jen blok fixedCosts.
-- ════════════════════════════════════════════════════════════════════
...
create table public.fixed_costs (
  ...
  -- První den měsíce, od kterého náklad platí.
  valid_from date not null,
  -- První den posledního měsíce, kdy platí. Prázdné = běží dál.
  valid_to   date,
  note       text,
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),

  -- Konec dřív než začátek by znamenal náklad, který nikdy neplatí.
  constraint fixed_costs_poradi check (valid_to is null or valid_to >= valid_from)
);

create index fixed_costs_obdobi_idx on public.fixed_costs (valid_from, valid_to);
