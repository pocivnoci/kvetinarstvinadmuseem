...
--  Moje výplata
--
--  Floristka pořád nakupuje do krámu a na konci měsíce neví, kde je její
--  výplata. Admin teď drží, kolik si chce měsíčně brát (settings.owner_pay),
--  a kolik si opravdu vyplatila (owner_payouts). Z toho počítá, kolik smí
--  tento týden utratit ve velkoobchodě, aby na výplatu zbylo.
--
--  Výplata není výdaj krámu: zisk se jí nemění, jen je vidět, kolik
--  z něj skončilo u majitelky a kolik zůstalo v krámu.
--
--  Mimochodem se tu opravuje sazba DPH v nákupu. Admin ji má v Nastavení
--  od 22. 9. 2026, ale databáze pro ni neměla sloupec — uložená hodnota se
--  tiše zahodila a po obnovení stránky byla zpátky výchozích 21 %.
--
--  Obě funkce jsou znovu v celku, aby se daly přečíst na jednom místě.
--  Proti 0005 přibyl blok payouts a dvě pole nastavení.
-- ════════════════════════════════════════════════════════════════════

alter table public.settings
  add column owner_pay numeric(12, 2) not null default 0 check (owner_pay >= 0),
  add column purchase_vat_rate numeric(5, 2) not null default 21 check (purchase_vat_rate >= 0);

create table public.owner_payouts (
  id         text primary key default gen_random_uuid()::text,
  -- Kdy se peníze převedly.
  paid_on    date not null,
  -- Za který měsíc (první den měsíce). Za září se často platí až v říjnu.
  for_month  date not null check (extract(day from for_month) = 1),
...
create index owner_payouts_for_month_idx on public.owner_payouts (for_month);

create trigger owner_payouts_touch before update on public.owner_payouts
  ...
  select jsonb_build_object(
    ...
    'fixedCosts', coalesce((
      select jsonb_agg(jsonb_strip_nulls(jsonb_build_object(
        ...
        'from', to_char(fc.valid_from, 'YYYY-MM'),
        'to', to_char(fc.valid_to, 'YYYY-MM'),
        'note', fc.note
      )) order by fc.amount desc)
      from public.fixed_costs fc), '[]'::jsonb),

    'payouts', coalesce((
      select jsonb_agg(jsonb_strip_nulls(jsonb_build_object(
        'id', op.id,
        'date', to_char(op.paid_on, 'YYYY-MM-DD'),
        'forMonth', to_char(op.for_month, 'YYYY-MM'),
        'amount', op.amount,
        'note', op.note
      )) order by op.paid_on desc)
      from public.owner_payouts op), '[]'::jsonb),
    ...
    'settings', (
      select jsonb_build_object(
        ...
        'vatRate', g.vat_rate,
        'purchaseVatRate', g.purchase_vat_rate,
        'laborFee', g.labor_fee,
        'wrapFee', g.wrap_fee,
        'roundTo', g.round_to,
        'ownerPay', g.owner_pay
...
begin
  ...
  delete from public.fixed_costs fc
   where not exists (
     select 1 from jsonb_array_elements(coalesce(doc->'fixedCosts', '[]'::jsonb)) e
      where e->>'id' = fc.id);

  insert into public.fixed_costs (id, name, amount, category, valid_from, valid_to, note)
  select e->>'id', e->>'name', coalesce((e->>'amount')::numeric, 0), e->>'category',
         to_date(e->>'from', 'YYYY-MM'),
         case when nullif(e->>'to', '') is null then null else to_date(e->>'to', 'YYYY-MM') end,
         e->>'note'
    from jsonb_array_elements(coalesce(doc->'fixedCosts', '[]'::jsonb)) e
  on conflict (id) do update set
    name = excluded.name, amount = excluded.amount, category = excluded.category,
    valid_from = excluded.valid_from, valid_to = excluded.valid_to, note = excluded.note;

  -- ── Výplaty sobě ──
  -- Jen když je dokument zná. Záložka otevřená ještě ze staré verze adminu
  -- pole payouts neposílá — a bez téhle podmínky by jejím uložením všechny
  -- zapsané výplaty zmizely.
  if doc ? 'payouts' then
    delete from public.owner_payouts op
     where not exists (
       select 1 from jsonb_array_elements(doc->'payouts') e
        where e->>'id' = op.id);

    insert into public.owner_payouts (id, paid_on, for_month, amount, note)
    select e->>'id', (e->>'date')::date, to_date(e->>'forMonth', 'YYYY-MM'),
           (e->>'amount')::numeric, e->>'note'
      from jsonb_array_elements(doc->'payouts') e
    on conflict (id) do update set
      paid_on = excluded.paid_on, for_month = excluded.for_month,
  ...
  update public.settings set
    ...
    purchase_vat_rate = coalesce((doc->'settings'->>'purchaseVatRate')::numeric, purchase_vat_rate),
    ...
    owner_pay      = coalesce((doc->'settings'->>'ownerPay')::numeric, owner_pay)
  where id;
